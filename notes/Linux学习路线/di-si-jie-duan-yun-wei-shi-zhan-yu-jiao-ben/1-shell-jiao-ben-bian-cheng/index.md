...
## 九、进阶：lint、参数解析、并行与健壮性

### 9.1 shellcheck：把严格模式落到工具
...
| 高频规则 | 含义 | 正确写法 |
|:---------|:-----|:---------|
| SC2086 | 变量未加引号（分词风险） | `echo "$var"` |
| SC2164 | `cd` 失败未检查 | `cd "$dir" || exit 1` |
| SC2046 | 命令替换未加引号 | `ls -l "$(pwd)"` |
| SC2148 | 缺少 shebang | 文件首行加 `#!/bin/bash` |

```bash
# 修复典型问题
cd "$SCRIPT_DIR" || exit 1        # SC2164
echo "文件: $file"                 # SC2086
result="$(grep foo "$f")"          # SC2046
```

> 💡 严格模式（`set -euo pipefail`）+ `shellcheck` 双保险：前者运行时兜底，后者静态扫描把隐患挡在发布前。

### 9.2 getopts：命令行参数解析

```bash
# 替代手写 $1/$2，支持 -u 带值、-v 开关
usage() { echo "用法: $0 [-u 用户名] [-v]"; exit 1; }

VERBOSE=0
while getopts "u:v" opt; do
  case "$opt" in
    u) USER="$OPTARG" ;;      # -u 后的值存入 OPTARG
    v) VERBOSE=1 ;;
    *) usage ;;
  esac
done
shift $((OPTIND - 1))         # ⭐ 跳过已解析选项，剩下的是位置参数

echo "用户: ${USER:-默认}  剩余参数: $*"
```

### 9.3 并行提速：& 与 wait、xargs -P
...
for f in *.log; do
  compress "$f" &
  pids+=("$!")
  [[ ${#pids[@]} -ge 4 ]] && { wait "${pids[@]}"; pids=(); }   # 每批最多 4 个
done
wait

# xargs -P 更简单的并行（关联 2.3 §7.5）
find . -name "*.gz" -print0 | xargs -0 -P 4 -I{} gzip -d {}
```

### 9.4 健壮性：trap ERR 与 flock

```bash
# trap ERR：任何命令失败立即走 handler（比 set -e 多一个收尾钩子）⭐
set -Eeuo pipefail            # -E 让 ERR 陷阱在函数/子 shell 中也生效
trap 'echo "第 $LINENO 行出错，退出码 $?"' ERR

# flock 文件锁：防止脚本被重复运行 ⭐
exec 9>/var/lock/myscript.lock
flock -n 9 || { echo "脚本已在运行，退出"; exit 1; }

# mktemp + trap 清理（§六 已有基础，这里强调组合）
readonly TMPD=$(mktemp -d)
trap 'rm -rf "$TMPD"' EXIT
```

> 💡 `flock -n` 对 **3.5 cron 里的长任务** 尤其重要：防止上一次没跑完、下一次又启动，导致重复处理。

### 9.5 子 shell 与变量隔离

```bash
# ( ) 子 shell：括号内修改不影响父 shell ⭐
x=1
(x=99; echo "子 shell: $x")    # 子 shell: 99
echo "父 shell: $x"            # 父 shell: 1
...
# export 只向下传递：子进程可见，父/兄弟进程看不到
export Z=10
( echo "子进程见 Z=$Z" )        # 子进程见 Z=10

# source 与直接执行的差别
#   source script.sh  ：当前 shell 运行，变量/函数留在本 shell
#   ./script.sh       ：开子进程运行，改动不影响父 shell
source deploy.sh       # 共享变量；./deploy.sh 则隔离

# $$ vs $BASHPID
echo "$$ vs $BASHPID"           # 脚本内一致；子 shell 里 $$ 不变、$BASHPID 变 ⭐
```
