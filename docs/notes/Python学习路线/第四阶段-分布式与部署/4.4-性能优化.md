...
permalink: /notes/Python学习路线/di-si-jie-duan-fen-bu-shi-yu-bu-shu/4-xing-neng-you-hua/
---
# 性能优化
...
2. 用 cProfile 定位 CPU 热点，用 line_profiler 定位具体行
3. 用 lru_cache、数据去重、异步并发等手段优化
4. 用 timeit / AP 压测对比优化前后的 P95 延迟
5. 用 memory_profiler 检查内存是否有泄漏
