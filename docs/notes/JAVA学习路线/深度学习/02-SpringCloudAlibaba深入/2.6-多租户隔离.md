...
permalink: >-
  /notes/JAVA学习路线/shen-du-xue-xi/02-spring-cloud-alibaba-shen-ru/6-duo-zu-hu-ge-li/
---
# 多租户隔离深入学习文档
...
#### 模型三：共享表（Shared Tables with tenant_id）
...
| 对比维度 | 独立数据库 | 共享库独立 Schema | 共享表（tenant_id） |
|---------|-----------|------------------|-------------------|
...
| SQL 层 | 自动追加 tenant_id 条件 | MyBatis/JPA 拦截器 |
| 接口层 | 校验资源归属 | AOP 切面 |
...
| 数据库（行级） | 共享表 + tenant_id | 高 | 中 | 中（索引优化后低） |
...
|------|--------------|---------|--------|---------|
| SaaS 中小客户 | 共享表 + tenant_id | Key 前缀 | Header 传递 | AOP 校验 |
...
> - Hibernate 多租户: https://docs.jboss.org/hibernate/orm/6.2/userguide/html_single/Hibernate_User_Guide.html#multitenacy
